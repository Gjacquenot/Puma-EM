...
su -c 'yum -y install python-devel gcc-c++ libgfortran gcc-gfortran libstdc++-devel compat-gcc-34-g77 compat-libstdc++-33 openmpi openmpi-libs openmpi-devel scipy numpy python-matplotlib-tk python-matplotlib blitz blitz-devel'
# relinking libg2c.so: bug in Fedora Core 8
echo " Root password for correcting a Fedora Core 8 link bug for libg2c.so "
su -c 'ln -s /usr/lib/gcc/i386-redhat-linux/3.4.6/libg2c.so /usr/lib/libg2c.so'
